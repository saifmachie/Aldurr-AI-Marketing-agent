...
async function getExportUrl(designId) {
...
async function exportDesignPng(designId, destPath) {
  // Downloads a local copy for the review batch. Note: the URL this fetches
  // is short-lived — do NOT persist it. Instagram's publish step (which can
  // happen months after this runs, for evergreen posts scheduled late in a
  // quarter) needs a fresh URL, so it calls getExportUrl(designId) again
  // right before publishing rather than reusing anything from here.
  const url = await getExportUrl(designId);
...
module.exports = { uploadAsset, autofillDesign, exportDesignPng, getExportUrl };
