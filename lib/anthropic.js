...
function parseJSON(text) {
  ...
  // Normal case: a complete ```json ... ``` fence.
  const closed = trimmed.match(/^```(?:json)?\s*([\s\S]*?)\s*```$/);
  if (closed) return JSON.parse(closed[1].trim());
  // Response got cut off before a closing fence (e.g. hit a token-limit
  // ceiling mid-output) — strip just the opening marker and try anyway.
  // If the JSON itself is genuinely truncated this still throws, but with
  // a much clearer "Unexpected end of JSON input" than a raw backtick.
  const openOnly = trimmed.match(/^```(?:json)?\s*([\s\S]*)$/);
  const raw = openOnly ? openOnly[1] : trimmed;
